...
class BaselineTrainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "coco":
            ...
            #ignore the last "others" class for visdrone
...
class UBTeacherTrainer(DefaultTrainer):
    ...
    # ================== Pseduo-labeling ==================
    # =====================================================
    def threshold_bbox(self, det_bbox_inst, thres=0.7):
        valid_map = det_bbox_inst.scores > thres

        # create instances containing boxes and gt_classes
        image_shape = det_bbox_inst.image_size
        new_bbox_inst = Instances(image_shape)

        # create box
        new_bbox_loc = det_bbox_inst.pred_boxes.tensor[valid_map, :]
        new_boxes = Boxes(new_bbox_loc)

        # add boxes to instances
        new_bbox_inst.gt_boxes = new_boxes
        new_bbox_inst.gt_classes = det_bbox_inst.pred_classes[valid_map]
        new_bbox_inst.scores = det_bbox_inst.scores[valid_map]

        return new_bbox_inst

    def process_pseudo_label(self, proposals_unsup_k, cur_threshold, psedo_label_method=""):
        list_instances = []
        num_proposal_output = 0.0
        for proposal_bbox_inst in proposals_unsup_k:
            # thresholding
        ...
        num_proposal_output = num_proposal_output / len(proposals_unsup_k)
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            proposals_unsup_w = [result["instances"] for result in results_unlabeled_data_w]
            pesudo_gt_unsup_w, avg_pseudo_gtboxes = self.process_pseudo_label(
                proposals_unsup_w, cur_threshold, "thresholding"
            )
            #experiment.log_metric("avg_no_pseudo_gt_boxes", avg_no_pseudo_gt_boxes, step=self.iter)

            # add pseudo-label to unlabeled data

            unlabel_data_s = self.add_label(unlabel_data_s, pesudo_gt_unsup_w)
            unlabel_data_w = self.add_label(unlabel_data_w, pesudo_gt_unsup_w)
