...
def get_sliding_window_patches(data_dict):
    height, width = data_dict["height"], data_dict["width"]
    new_boxes = np.zeros((0, 4), dtype=np.int32)
    h_range = np.arange(0, height, 1500)
    w_range = np.arange(0, width, 1500)
    if (height-h_range[-1]) < 320:
        h_range[-1] = height
    else:
        h_range = np.append(h_range, height)
    if (width-w_range[-1]) < 320:
        w_range[-1] = width
    else:
        w_range = np.append(w_range, width)
    for i in range(len(h_range)-1):
        for j in range(len(w_range)-1):
            new_boxes = np.append(new_boxes, np.array([w_range[j], h_range[i], w_range[j+1], h_range[i+1]]).reshape(1, -1), axis=0)
    return new_boxes
...
def load_dota_instances(dataset_name, data_dir, cfg, is_train, extra_annotation_keys=None):
...
def register_dota(dataset_name, data_dir, cfg, is_train):
    from pycocotools.coco import COCO
    metadata = {}

    # 1. register a function which returns dicts
    DatasetCatalog.register(dataset_name, lambda: load_dota_instances(dataset_name, data_dir, cfg, is_train))
